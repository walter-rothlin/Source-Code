...
class Fraction:
    ...
    def nenner(self, nenner_param):
        if nenner_param is None:
            ...
            try:
                self.__nenner = int(round(float(nenner_param)))
            except Exception:
                self.__nenner = 1
    ...
    def to_decimal(self, roundAfter=2):
        ...
        if roundAfter is not None:
            ...
            return round(self.zaehler / self.nenner, roundAfter)
        else:
    ...
    def shorten(self, divisor=None):
        ...
        if divisor == -1:
            if self.__zaehler < 0 and self.__nenner < 0:
                return self
            elif self.__zaehler < 0 and self.__nenner > 0:
