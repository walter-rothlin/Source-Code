...
class Fraction:
    ...
    def __init__(self, zaehler=0, nenner=1, bruch=None, bruch_str=None):
        ...
        self.nenner = nenner
        # Die Normalisierung der Vorzeichen wird laut Testfällen erst in der `shorten`-Methode erwartet.
        # Die Werte werden hier so gespeichert, wie sie übergeben wurden.
    ...
    def set_zaehler(self, zaehler_param):
        if zaehler_param is None:
            ...
            try:
                # Floats und Strings, die Floats repräsentieren, korrekt verarbeiten
                self.__zaehler = int(round(float(zaehler_param)))
            except (ValueError, TypeError):
                # Bei ungültigen Werten (z.B. "Hallo") auf 0 setzen
                self.__zaehler = 0
    ...
    def nenner(self, nenner_param):
        if nenner_param is None:
            ...
            try:
                nenner_val = int(round(float(nenner_param)))
                if nenner_val == 0:
                    # Nenner 0 wird zu 1, wie in der Docstring beschrieben
                    self.__nenner = 1
                else:
                    self.__nenner = nenner_val
            except (ValueError, TypeError):
                # Bei ungültigen Werten (z.B. "Hallo") auf 1 setzen
                self.__nenner = 1
    ...
    def __lt__(self, other):  # <
        ...
        """
        return self.to_decimal(roundAfter=None) < other.to_decimal(roundAfter=None)
    ...
    def to_decimal(self, roundAfter=2):
        ...
        """
        value = self.__zaehler / self.__nenner
    ...
    def shorten(self, divisor=None):
        ...
        # Wenn divisor ungültig ist (0, 1) oder kein gemeinsamer Teiler ist, den ggT verwenden
        if divisor == 0 or divisor == 1 or self.__zaehler % divisor != 0 or self.__nenner % divisor != 0:
            divisor = math.gcd(self.__zaehler, self.__nenner)
            if divisor == 0: # Falls Zähler 0 ist, kann gcd 0 sein
                divisor = 1
    ...
    def expand(self, factor=None):
        ...
        if self.__zaehler < 0 and self.__nenner < 0:
...
if __name__ == '__main__':
    ...
    def TEST_reciprocal_to_decimal_shorten_expand(verbal=False):
        ...
        205|to_decimal|[2/3]     |None   |     |0.6666666666666666
