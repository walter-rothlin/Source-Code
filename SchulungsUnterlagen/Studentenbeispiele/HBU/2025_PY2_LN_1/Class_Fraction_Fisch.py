...
# 24-Sep-2025   Ricardo Fisch   Korrigiertes Skript ohne die Test zu ändern
# ------------------------------------------------------------------
import math
...
class Fraction:
    ...
    def __init__(self, zaehler=0, nenner=1, bruch=None, bruch_str=None):
        ...
        if Fraction.do_trace:
            print(f'_init__(self, zaehler={zaehler}, nenner=1, bruch=None, bruch_str=None):')  ## bei nenner=nenner und nicht 1 wird beser ausgelesen

## Fix: Hilft für parsing, und rundet und konvertiert (Neue Spezifikation)
    def round_to_int(n, default):
        if n is None: return default
        if insinstance(n, str):
            try: n = float(n)
            except ValueError: return default
            if isinstance(n, float): return int(round(n))
