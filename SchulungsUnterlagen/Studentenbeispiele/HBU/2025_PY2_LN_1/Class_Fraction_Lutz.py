...
# ------------------------------------------------------------------
# Summary of Changes (Sep 2025):
# - __init__:    Fehler: zaehler falsch gesetzt → Fix: korrekt zuweisen, Floats runden, Vorzeichen beibehalten
# - bruch_str:   Fehler: Parsing unvollständig → Fix: ':' und Einzelwerte unterstützt, immer int
# - Getter/Setter: Fehler: falsche Defaults → Fix: zaehler=0, nenner=1 bei ungültig, Floats runden
# - shorten():   Fehler: falscher Divisor/ggt → Fix: ggt genutzt, Vorzeichen angepasst
# - expand():    Fehler: Nenner bei +faktor negativ → Fix: Nenner positiv erzwingen (nur bei +faktor)
# - to_decimal(): Fehler: Division verkehrt → Fix: zaehler/nenner, Rundung nach Param
# - Operatoren:  Fehler: Wertgleichheit statt Struktur → Fix: __eq__ prüft exakt Zähler/Nenner, andere mit Dezimalwert
#
# ------------------------------------------------------------------
import math
...
class Fraction:
    ...
    def __init__(self, zaehler=0, nenner=1, bruch=None, bruch_str=None):
        ...
        if Fraction.do_trace:
            print(f'_init__(self, zaehler={zaehler}, nenner={nenner}, bruch={bruch}, bruch_str={bruch_str}):')
        ...
        if bruch_str is not None:
            ...
            if len(parts) == 1 and parts[0] != '':
                ...
                zaehler, nenner = parts[0], parts[1]
    ...
    def set_zaehler(self, zaehler_param):
        if zaehler_param is None:
            ...
            try:
                self.__zaehler = int(round(float(zaehler_param), 0))
            except Exception:
                self.__zaehler = 0
    ...
    def nenner(self, nenner_param):
        if nenner_param is None:
            ...
            self.__nenner = nenner_param if nenner_param != 0 else 1
        else:
            try:
                val = int(round(float(nenner_param), 0))
                self.__nenner = val if val != 0 else 1
            except Exception:
                self.__nenner = 1
    ...
    def to_decimal(self, roundAfter=2):
        ...
        if roundAfter is not None:
            ...
            return round(self.__zaehler / self.__nenner, 2)
    ...
    def shorten(self, divisor=None):
        ...
        used_gcd = False
        if d == 0 or d == 1 or (self.__zaehler % d != 0 or self.__nenner % d != 0):
        ...
        self.__zaehler = int(self.__zaehler // d)
        self.__nenner = int(self.__nenner // d)

        if used_gcd or d > 0:
            if self.__nenner < 0 and self.__zaehler >= 0:
                self.__zaehler *= -1
                self.__nenner *= -1
            if self.__nenner < 0 and self.__zaehler < 0:
                self.__zaehler = abs(self.__zaehler)
                self.__nenner = abs(self.__nenner)
    ...
    def expand(self, factor=None):
        ...
        # Fix: Nur bei positivem Faktor sicherstellen,
        # dass der Nenner positiv bleibt
        if f > 0:
            if self.__nenner < 0 and self.__zaehler >= 0:
                self.__zaehler *= -1
                self.__nenner  *= -1
