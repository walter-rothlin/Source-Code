...
SET @Candidate_Firstname  := 'Gianluca';    -- Durch ihren Vornamen ersetzen
SET @Candidate_Name       := 'Petroni';   -- Durch ihren Namen ersetzen
SET @Class                := 'BWI-A24';   -- z.B. BWI-A24
...
SELECT COUNT(*) AS Anzahl_Datensaetze FROM `Personen_Liste`;
-- Ergebnis: 1169 (gleich wie 1.1)

SELECT COUNT(*) AS Anzahl_Wangen FROM `Personen_Liste` WHERE `PLZ_Ort` = '8855 Wangen';
-- Ergebnis: 703 (gleich wie 1.2)

SELECT COUNT(DISTINCT `PLZ_Ort`) AS Anzahl_PLZ_Ort FROM `Personen_Liste` WHERE `PLZ_Ort` IS NOT NULL;
-- Ergebnis: 82 (gleich wie 1.3)
...
UPDATE `personen`
SET `Hausnummer` = CASE
                       WHEN REGEXP_INSTR(`Strasse`, '[0-9]') > 0
                       THEN TRIM(SUBSTRING(`Strasse`, REGEXP_INSTR(`Strasse`, '[0-9]')))
                       ELSE NULL
                   END;
...
SET `Strasse` = CASE
                    WHEN REGEXP_INSTR(`Strasse`, '[0-9]') > 0
                    THEN TRIM(LEFT(`Strasse`, REGEXP_INSTR(`Strasse`, '[0-9]') - 1))
                    ELSE `Strasse`
                END;
SET SQL_SAFE_UPDATES = 1;
...
CREATE OR REPLACE VIEW `Personen_Liste` AS
    SELECT
        ...
        `Vorname`,
        `Nachname`,
        TRIM(CONCAT(COALESCE(`Strassenname`, ''), ' ', COALESCE(`Hausnummer`, ''))) AS `Strasse`,
        `PLZ_Ort`,
        `Tel_Nr`,
        `eMail`
    FROM `personen`;
...
CREATE TABLE IF NOT EXISTS `Orte` (
    ...
    PRIMARY KEY (`ID`),
    UNIQUE KEY `PLZ_Ort_UQ` (`PLZ`, `Ortsname`)
);
...
SET p.`Orte_FK` = o.`ID`
WHERE p.`PLZ_Ort` IS NOT NULL AND p.`PLZ_Ort` != '';
SET SQL_SAFE_UPDATES = 1;
...
CREATE OR REPLACE VIEW `Personen_Liste` AS
    SELECT
        ...
        p.`Vorname`,
        p.`Nachname`,
        TRIM(CONCAT(COALESCE(p.`Strassenname`, ''), ' ', COALESCE(p.`Hausnummer`, ''))) AS `Strasse`,
...
CREATE OR REPLACE VIEW `Personen_Liste` AS
    SELECT
        ...
        p.`Nachname`,
        TRIM(CONCAT(COALESCE(p.`Strassenname`, ''), ' ', COALESCE(p.`Hausnummer`, ''))) AS `Strasse`,
        -- p.`PLZ_Ort` AS `PLZ_Ort_Old`,   -- auskommentiert nach erfolgreicher Verifikation
        CONCAT(o.`PLZ`, ' ', o.`Ortsname`)                                               AS `PLZ_Ort`,
        p.`Tel_Nr`,
        p.`eMail`
    FROM `personen` p
