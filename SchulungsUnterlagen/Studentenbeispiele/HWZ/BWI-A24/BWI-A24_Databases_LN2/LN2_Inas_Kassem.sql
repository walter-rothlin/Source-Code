...
SET @Candidate_Firstname  := 'Inas';    -- Durch ihren Vornamen ersetzen
SET @Candidate_Name       := 'Kassem';   -- Durch ihren Namen ersetzen
...
SELECT COUNT(DISTINCT PLZ_Ort) AS Anzahl_Distinct_PLZ_Ort
FROM Personen_Liste
WHERE PLZ_Ort IS NOT NULL;
-- Anzahl: 82

-- Flag: nur beim allerersten Lauf ist "Personen_Liste" noch eine Basistabelle.
SET @first_run = (
...
SET @sql = IF(@first_run > 0,
   'SELECT Strasse,
      ...
      FROM personen
      LIMIT 20',
   'SELECT ''bereits migriert - kein Split mehr noetig'' AS Hinweis');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@first_run > 0,
   ...
   'SELECT ''bereits migriert - keine Kontrolle noetig'' AS Hinweis');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @fk_exists = (
    ...
    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'personen'
      AND CONSTRAINT_NAME = 'fk_personen_orte' AND CONSTRAINT_TYPE = 'FOREIGN KEY'
);
