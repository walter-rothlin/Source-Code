...
SET @Candidate_Firstname  := 'Diego';     -- Durch ihren Vornamen ersetzen
SET @Candidate_Name       := 'Casetti';   -- Durch ihren Namen ersetzen
...
USE `pruefung_2`;
SET SQL_SAFE_UPDATES = 0;   -- erlaubt spätere Bulk-UPDATEs ohne PK in der WHERE-Clause
...
CREATE OR REPLACE VIEW `Personen_Liste` AS
    SELECT
        ...
        `p`.`eMail`
    FROM `personen` AS `p`
    LEFT JOIN `Orte` AS `o` ON `o`.`ID` = `p`.`Orte_FK`;
...
CREATE OR REPLACE VIEW `Personen_Liste` AS
    SELECT
        ...
        `p`.`Nachname`,
        TRIM(CONCAT(`p`.`Strassenname`, ' ', IFNULL(`p`.`Hausnummer`, ''))) AS `Strasse`,
        -- `p`.`PLZ_Ort` AS `PLZ_Ort_Old`,        -- auskommentiert: Migration verifiziert
        CONCAT(`o`.`PLZ`, ' ', `o`.`Ortsname`)   AS `PLZ_Ort`,
        `p`.`Tel_Nr`,
        `p`.`eMail`
    FROM `personen` AS `p`
...
-- Abschluss: Safe-Update-Mode wieder einschalten.
