-- ---------------------------------------------------------------------------------
-- LN2 - Databases : Normalisierung der "Personen_Liste"
-- Autor      : Görkem Taspinar
-- Klasse     : BWI-A24
-- Schema     : pruefung_2  (Default-Schema)
--
-- Re-runnable Migrations-Script (Test-Driven Approach). Voraussetzung:
-- pruefung_2_schema.sql und pruefung_2a_data.sql wurden ausgeführt.
-- Jeder struktur-/datenverändernde Schritt ist über einen information_schema-Check
-- abgesichert (PREPARE/EXECUTE) -> das Script ist mehrfach ausführbar (re-runnable).
-- ---------------------------------------------------------------------------------
-- -------------- Hier bitte ihre persönlichen Angaben erfassen --------------------

SET @Candidate_Firstname  := 'Görkem';    -- Durch ihren Vornamen ersetzen
SET @Candidate_Name       := 'Taspinar';  -- Durch ihren Namen ersetzen
...
SET FOREIGN_KEY_CHECKS = 0;   -- erlaubt re-runnable DROP/CREATE der abhängigen Objekte
...
SELECT COUNT(DISTINCT `PLZ_Ort`) AS anzahl_versch_plz_ort
FROM `Personen_Liste`
...
SET @stmt := IF(
  (SELECT COUNT(*) FROM information_schema.TABLES
     WHERE TABLE_SCHEMA='pruefung_2' AND TABLE_NAME='personen' AND TABLE_TYPE='BASE TABLE') = 0,
...
SET @stmt := IF(
  (SELECT COUNT(*) FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA='pruefung_2' AND TABLE_NAME='personen' AND COLUMN_NAME='Strasse') = 1,
...
SELECT COUNT(*)                  AS chk_2_3_total  FROM `Personen_Liste`;                               -- 1169
SELECT COUNT(*)                  AS chk_2_3_wangen FROM `Personen_Liste` WHERE `PLZ_Ort`='8855 Wangen'; -- 703
SELECT COUNT(DISTINCT `PLZ_Ort`) AS chk_2_3_plz    FROM `Personen_Liste` WHERE `PLZ_Ort` IS NOT NULL;   -- 82
...
SET @stmt := IF(
  (SELECT COUNT(*) FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA='pruefung_2' AND TABLE_NAME='personen' AND COLUMN_NAME='Hausnummer') = 0,
  'ALTER TABLE `personen` ADD COLUMN `Hausnummer` VARCHAR(45) NULL AFTER `Strasse`',
  'DO 0');
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;
...
-- Strasse und Hausnummer extrahieren
-- Hausnummer = ab dem ersten Token, das mit einer Ziffer beginnt, bis zum Ende
-- (deckt auch "Seestr. 69 b" ab; Strassen ohne Nummer wie "Kählhof" -> Hausnummer NULL).
SET @stmt := IF(
  (SELECT COUNT(*) FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA='pruefung_2' AND TABLE_NAME='personen' AND COLUMN_NAME='Strasse') = 1,
  'UPDATE `personen`
...
-- 3.3) Strassenname extrahieren (= der Rest der "Strasse" ohne die Hausnummer).
--      Hinweis: Im Original-Fragenscript fehlt die Nummer 3.3 (Sprung 3.2 -> 3.4);
--      hier als eigener Schritt ergänzt.
SET @stmt := IF(
  (SELECT COUNT(*) FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA='pruefung_2' AND TABLE_NAME='personen' AND COLUMN_NAME='Strasse') = 1,
  'UPDATE `personen`
      SET `Strasse` = TRIM(REGEXP_REPLACE(`Strasse`, '' [0-9].*$'', ''''))',
  'DO 0');
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;
...
SET @stmt := IF(
  (SELECT COUNT(*) FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA='pruefung_2' AND TABLE_NAME='personen' AND COLUMN_NAME='PLZ_Ort') = 1,
  'CREATE OR REPLACE VIEW `Personen_Liste` AS
...
--      Hinweis: PLZ ist NICHT eindeutig (z.B. 8855 Wangen / 8855 Nuolen) -> Surrogate-Key "ID" als PK.
CREATE TABLE IF NOT EXISTS `Orte` (
  `ID`       INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `PLZ`      VARCHAR(20)  NULL,
  `Ortsname` VARCHAR(100) NULL,
  PRIMARY KEY (`ID`),
...
SET @stmt := IF(
  ...
  'INSERT INTO `Orte` (`PLZ`,`Ortsname`)
     SELECT DISTINCT SUBSTRING_INDEX(`PLZ_Ort`,'' '',1)                  AS PLZ,
...
SET @stmt := IF(
  (SELECT COUNT(*) FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA='pruefung_2' AND TABLE_NAME='personen' AND COLUMN_NAME='Orte_FK') = 0,
...
SET @stmt := IF(
  (SELECT COUNT(*) FROM information_schema.COLUMNS
     WHERE TABLE_SCHEMA='pruefung_2' AND TABLE_NAME='personen' AND COLUMN_NAME='PLZ_Ort') = 1,
  'SELECT * FROM `Personen_Liste`
     WHERE `PLZ_Ort_Old` <> `PLZ_Ort`
        OR (`PLZ_Ort_Old` IS NULL) <> (`PLZ_Ort` IS NULL)',
  'DO 0');
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;
...
CREATE OR REPLACE VIEW `Personen_Liste` AS
  SELECT p.`ID`, p.`Anrede`, p.`Vorname`, p.`Nachname`,
         TRIM(CONCAT(p.`Strassenname`, ' ', COALESCE(p.`Hausnummer`, ''))) AS `Strasse`,
      -- p.`PLZ_Ort`                        AS `PLZ_Ort_Old`,   -- nach erfolgreicher Migration auskommentiert
         CONCAT(o.`PLZ`, ' ', o.`Ortsname`) AS `PLZ_Ort`,
         p.`Tel_Nr`, p.`eMail`
  FROM `personen` p
  LEFT JOIN `Orte` o ON p.`Orte_FK` = o.`ID`;
...
SET @stmt := IF(
  (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
     WHERE TABLE_SCHEMA='pruefung_2' AND TABLE_NAME='personen' AND CONSTRAINT_NAME='Orte_FK') = 0,
