...
SET @Candidate_Firstname  := 'Noa';    -- Durch ihren Vornamen ersetzen
SET @Candidate_Name       := 'Djelmo';   -- Durch ihren Namen ersetzen
...
-- ---------------------------------------------------------------------------------
-- Schema explizit setzen, da mehrere Schemen vorhanden sind
USE `pruefung_2`;
...
SELECT COUNT(DISTINCT PLZ_Ort) AS Anzahl_Verschiedene_PLZ_Ort FROM Personen_Liste WHERE PLZ_Ort IS NOT NULL;
...
SET `Hausnummer` =
    CASE
        WHEN `Strasse` REGEXP '[0-9]+[a-zA-Z]?$'
            THEN TRIM(REGEXP_SUBSTR(`Strasse`, '[0-9]+[a-zA-Z]?$'))
        ELSE NULL
    END
WHERE `Hausnummer` IS NULL;   -- re-runable: bereits gesetzte Werte nicht überschreiben
 
UPDATE `personen`
SET `Strasse` =
    CASE
        WHEN `Strasse` REGEXP '[0-9]+[a-zA-Z]?$'
            THEN TRIM(REGEXP_REPLACE(`Strasse`, '[0-9]+[a-zA-Z]?$', ''))
        ELSE `Strasse`
    END;
...
SELECT
    ...
    `Vorname`,
    `Nachname`,
    TRIM(CONCAT_WS(' ', `Strassenname`, `Hausnummer`)) AS `Strasse`,
    `PLZ_Ort`,
    `Tel_Nr`,
    `eMail`
FROM `personen`;
...
CREATE TABLE IF NOT EXISTS `Orte` (
    ...
    PRIMARY KEY (`ID`),
    UNIQUE KEY `UK_Orte_PLZ_Ort` (`PLZ`, `Ortsname`)
);
...
SELECT
    ...
    p.`Vorname`,
    p.`Nachname`,
    TRIM(CONCAT_WS(' ', p.`Strassenname`, p.`Hausnummer`)) AS `Strasse`,
...
SELECT
    ...
    p.`Nachname`,
    TRIM(CONCAT_WS(' ', p.`Strassenname`, p.`Hausnummer`)) AS `Strasse`,
    -- p.`PLZ_Ort` AS `PLZ_Ort_Old`,   -- auskommentiert, da Migration verifiziert
    CONCAT_WS(' ', o.`PLZ`, o.`Ortsname`)                  AS `PLZ_Ort`,
    p.`Tel_Nr`,
    p.`eMail`
FROM `personen` p
