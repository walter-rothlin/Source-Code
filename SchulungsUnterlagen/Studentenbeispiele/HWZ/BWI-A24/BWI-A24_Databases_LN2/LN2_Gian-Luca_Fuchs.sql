...
-- 30-Jun-2026   Gian-Luca Fuchs     Solve LN2
-- ---------------------------------------------------------------------------------
-- -------------- Hier bitte ihre persönlichen Angaben erfassen --------------------

SET @Candidate_Firstname  := 'Gian-Luca';    -- Durch ihren Vornamen ersetzen
SET @Candidate_Name       := 'Fuchs';   -- Durch ihren Namen ersetzen
...
UPDATE personen
SET
    Hausnummer = REGEXP_SUBSTR(Strasse, '[0-9]+[a-z]?( [a-z])?$'),
    Strasse    = TRIM(REGEXP_REPLACE(Strasse, '\\s*[0-9]+[a-z]?( [a-z])?$', ''))
...
CREATE OR REPLACE VIEW Personen_Liste AS
    SELECT
        ...
        CASE
            WHEN Hausnummer IS NOT NULL THEN CONCAT(Strassenname, ' ', Hausnummer)
            ELSE Strassenname
        END AS Strasse,
...
-- PLZ allein ist kein eindeutiger Schlüssel (z.B. 8855 gilt für Wangen UND Nuolen),
-- daher wird ein Auto-Increment PK verwendet.
...
SELECT DISTINCT SUBSTRING(PLZ_Ort, 1, 4), SUBSTRING(PLZ_Ort, 6)
FROM personen
...
UPDATE personen p
JOIN Orte o ON o.PLZ = SUBSTRING(p.PLZ_Ort, 1, 4)
           AND o.Ortsname = SUBSTRING(p.PLZ_Ort, 6)
SET p.Orte_FK = o.ID
...
CREATE OR REPLACE VIEW Personen_Liste AS
    SELECT
        ...
        CASE
            WHEN p.Hausnummer IS NOT NULL THEN CONCAT(p.Strassenname, ' ', p.Hausnummer)
            ELSE p.Strassenname
        END AS Strasse,
