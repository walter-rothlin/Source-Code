...
SET @Candidate_Firstname  := 'Ken';       -- Durch ihren Vornamen ersetzen
SET @Candidate_Name       := 'Osorio';    -- Durch ihren Namen ersetzen
...
CREATE OR REPLACE VIEW `Personen_Liste` AS
    SELECT `ID`, `Anrede`, `Vorname`, `Nachname`,
           CONCAT_WS(' ', `Strassenname`, NULLIF(`Hausnummer`, '')) AS `Strasse`,
...
CREATE OR REPLACE VIEW `Personen_Liste` AS
    SELECT p.`ID`, p.`Anrede`, p.`Vorname`, p.`Nachname`,
           CONCAT_WS(' ', p.`Strassenname`, NULLIF(p.`Hausnummer`, ''))  AS `Strasse`,
...
-- Erwartet: 0 Zeilen → Migration korrekt
