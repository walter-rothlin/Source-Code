...
SET @Candidate_Firstname  := 'Jonas';
SET @Candidate_Name       := 'Russi';
SET @Class                := 'BWI-A24';

SET @Test_Name            := 'DB_Prüfung_2c';  -- NICHT ändern
SET @Test_Max_Punkte      := 15;               -- NICHT ändern

-- Safe-Update-Mode aus, damit die UPDATEs (3.2 & 4.4) überall durchlaufen
...
SELECT COUNT(*)                  AS Anzahl_Datensaetze            FROM Personen_Liste;                                   -- 1169
SELECT COUNT(*)                  AS Anzahl_Wangen                 FROM Personen_Liste WHERE PLZ_Ort = '8855 Wangen';     -- 703
SELECT COUNT(DISTINCT PLZ_Ort)   AS Anzahl_verschiedene_PLZ_Ort   FROM Personen_Liste WHERE PLZ_Ort IS NOT NULL;         -- 82
...
UPDATE `personen`
SET `Hausnummer` = REGEXP_SUBSTR(`Strasse`, '[0-9]+ *[a-zA-Z]?$'),
    `Strasse`    = TRIM(REGEXP_REPLACE(`Strasse`, '[0-9]+ *[a-zA-Z]?$', ''));
...
SELECT
    ...
    p.`Nachname`,
    TRIM(CONCAT(p.`Strassenname`, ' ', COALESCE(p.`Hausnummer`, ''))) AS `Strasse`,
 -- p.`PLZ_Ort`                              AS `PLZ_Ort_Old`,   -- nach Verifizierung entfernt
    CONCAT(o.`PLZ`, ' ', o.`Ortsname`)       AS `PLZ_Ort`,
    p.`Tel_Nr`,
    p.`eMail`
FROM `personen` p
