-- ---------------------------------------------------------------------------------------------
-- Filename: 01_Sol_DDL_DML_Eigenes_Schema.sql
-- Source  : https://raw.githubusercontent.com/walter-rothlin/Source-Code/master/Datenbanken/Scripts/01_Sol_DDL_DML_Eigenes_Schema.sql
-- ---------------------------------------------------------------------------------------------
--
...
DROP VIEW IF EXISTS `Adressen_Orte_Liste`; 
CREATE VIEW `Adressen_Orte_Liste` AS
	SELECT 
		`adr`.`ID`                                       AS `ID`,
		`adr`.`Strasse`                                  AS `Strasse`,
		`adr`.`Hausnummer`                               AS `Hausnummer`,
		CONCAT(`adr`.`Strasse`, ' ', `adr`.`Hausnummer`) AS `Strasse_Nr`,
		`adr`.`Orte_ID`                                  AS `Ort_ID`,
		`o`.`PLZ`                                        AS `PLZ`,
		`o`.`Name`                                       AS `Orts_Name`,
		`o`.`Land_code`                                  AS `Land_Code`,
		IF (`o`.`Land_code` = 'CH',
			CONCAT(`o`.`PLZ`, ' ', `o`.`Name`),
			CONCAT(`o`.`Land_code`, '-', `o`.`PLZ`, ' ', `o`.`Name`)) AS `PLZ_Ort`
	FROM `Adressen` AS `adr`
	INNER JOIN `Orte` AS `o` ON `adr`.`Orte_ID` = `o`.`ID`;
