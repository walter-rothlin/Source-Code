...
# 04-Sep-2023   Walter Rothlin      input_float mit min, max und Fehlermeldung als optionale Parameter
...
def input_float(prompt, max_tries=3):
    tries = 0
    error = True
...
def rad2grad(rad_value, trace=True):
    ...
    grad_value = rad_value * 180 / pi  # grad = rad*180/pi
    if trace:
...
def grad2rad(grad_value, trace=True):
    ...
    rad_value = grad_value * pi / 180  # rad  = grad*pi/180
    if trace:
