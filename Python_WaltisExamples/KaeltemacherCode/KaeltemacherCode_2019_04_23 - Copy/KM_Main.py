...
while (doLoop):
    ...
    spannungVerdichter_1_ok           = hsrKaelteMaschine.getSpannungVerdichter_1_ok()
    spannungVerdichter_2_ok           = hsrKaelteMaschine.getSpannungVerdichter_2_ok()
    einschaltVerzoegerungAktiv                   = hsrKaelteMaschine.getEinschaltVerzoegerung()
	
    verdichterReady = (spannungVerdichter_1_ok and spannungVerdichter_2_ok)
    ...
    verdichter_1.setValues(f1x(qGefiltert),((verdichterReady) or (ndOff)),einschaltVerzoegerungAktiv)
	
    # ------------
    # Verdichter_2
    # ------------
    hdOff = hochDruckOff.setState(hochdruck)
    verdichter_2.setValues(f2x(qGefiltert),((verdichterReady) or (hdOff)),einschaltVerzoegerungAktiv)
    ...
    # Absperrventil_EVR_3
    # -------------------
    if (verdichterReady or ((verdichter_1.getVerdichterOn() == False) and (verdichter_2.getVerdichterOn() == False))):
        absperrVentilEVR3_closed = True
    else:
